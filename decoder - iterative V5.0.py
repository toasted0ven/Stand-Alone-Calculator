...
main - deals with brackets, calling functions and calling order of ops
...
def main(list_x):
    ...
    while num_open > 0:
        ...
        num_open = list_x.count('(')
        last_open = list_search_r(list_x, '(')
        ...
        if len(mini_list) == 0:
            ...
            if len(number) == 3:
                # we have a start and finshing bracket
                try:
            ...
            if last_open:
                for i in range(last_open + 2, next_close + 1):
...
def testing(test):
    ...
    if test['main']:
        print('testing main')
        print('calling main([5 + 5]) ' + str(main(['5', '+', '5'])))
        print('(5 x (6 - 2 x 2)) = 10 - Check: ' + str(main(['(', '5', 'x', '(', '6', '-', '2', 'x', '2', ')', ')'])))
        print('sin(30) = 0.5 - Check: ' + str(main(['sin', '(', '0.523599', ')'])))
